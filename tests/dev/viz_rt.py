...
def rt_cdot(psds):
    return rp.viz.rt.matrix.cdot(Prop.hist_x, Prop.hist_y, psds)

def rt_vec(psds):
    return rp.viz.rt.matrix.vec(Prop.hist_x, Prop.hist_y, psds)

def rt_cvec(psds):
    return rp.viz.rt.matrix.cvec(Prop.hist_x, Prop.hist_y, psds)


def speed(sink):
    samps = sink.read(Prop.num_samps)
    snips = rp.utils.stft.stft(samps, Prop.nfft, Prop.overlap, Prop.window)
    psds = rp.utils.stft.psd(snips, Prop.Fs, Prop.vbw)
    ...
    # results.append(test(psds, count, rt_cdot, "cdot"))
    results.append(test(psds, count, rt_vec, "vec"))
    results.append(test(psds, count, rt_cvec, "cvec"))
...
def loop(sink):
    ...
    for samps in sink(Prop.num_samps):
        ...
        im = rp.viz.rt.Persistent(im, psds, Prop.hist_x, Prop.hist_y)
